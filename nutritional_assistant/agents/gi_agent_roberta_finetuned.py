...
class GIAnalysisAgentRoBERTaFinetuned(BaseAgent):
    """
    ...
    def __init__(self):
        ...
        super().__init__(name="GI Analysis Agent (Finetuned)")
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained("zoya-hammadk/nutrivision-roberta")
        ...
        self.gi_ranges = {
            "LABEL_0": 10.0,  # Low GI (0-20)
            "LABEL_1": 30.0,  # Low-Medium GI (21-40)
            "LABEL_2": 50.0,  # Medium GI (41-60)
            "LABEL_3": 70.0,  # Medium-High GI (61-80)
            "LABEL_4": 90.0   # High GI (81-100)
        }
    ...
    def get_gi_value(self, ingredient: str) -> float:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Just return the probability value directly
                return float(probs[0][0].item())
            
        except Exception as e:
            print(f"Error getting GI value for {ingredient}: {str(e)}")
            return 0.5  # Default to middle probability if error
    ...
    def analyze_glycemic_load(self, glycemic_load: float) -> Dict[str, Any]:
        """
        Analyze the glycemic load and provide interpretation.
        
        Args:
            glycemic_load (float): Calculated glycemic load
        ...
        if glycemic_load < 10:
            category = "Low"
            impact = "The glycemic load indicates a minimal impact on blood sugar levels"
            recommendation = "This recipe is suitable for maintaining stable blood sugar levels"
        elif glycemic_load < 20:
            category = "Medium"
            impact = "The glycemic load indicates a moderate impact on blood sugar levels"
            recommendation = "Consume in moderation, especially for individuals managing blood sugar levels"
        else:
            category = "High"
            impact = "The glycemic load indicates a significant impact on blood sugar levels"
    ...
    def process(self, recipes: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Process a list of recipes and determine their glycemic impact.
        ...
        try:
            ...
            recipe = recipes[0]  # Process first recipe
            
            # Extract ingredients
            ingredients = self.extract_ingredients(recipe)
            if not ingredients:
                return {"error": "Failed to extract ingredients"}
                
            # Calculate glycemic load
            glycemic_load = self.calculate_glycemic_load(ingredients)
            
            # Analyze results
            analysis = self.analyze_glycemic_load(glycemic_load)
